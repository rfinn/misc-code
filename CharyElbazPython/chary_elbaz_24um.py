...
from pylab import *
import atpy
chary_path='/Users/rfinn/CharyElbazPython/'
...
def chary_elbaz_24um(redshift,SmicroJy, H0=70., o_m=0.3, o_l=0.7):
    ...
    for igal in range(ngals):
        ...
        f12_rf = iras(ce.lamb, Snu[igal,:]/(1e0+redshift[igal]),band=12)
        f25_rf = iras(ce.lamb, Snu[igal,:]/(1e0+redshift[igal]),band=25)
        f60_rf = iras(ce.lamb, Snu[igal,:]/(1e0+redshift[igal]),band=60)
        f100_rf = iras(ce.lamb, Snu[igal,:]/(1e0+redshift[igal]),band=100)
